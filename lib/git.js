...
(function(git) {
  ...
  git.getUser = function getUser() {
    return childProcess.exec('"git config --get ' + git.config.user + '"', {
      cwd: git.target
    }).then(function(out) {
  ...
  git.getPass = function getPass() {
    return childProcess.exec('"git config --get ' + git.config.pass + '"', {
      cwd: git.target
    }).then(function(out) {
      if(out.stderr !== void 0 && out.stderr !== '') {
        return Promise.reject(out.stderr);
  ...
  git.getBranch = function getBranch() {
    return childProcess.exec('"git config --get ' + git.config.branch + '"', {
      cwd: git.target
    }).then(function(out) {
      if(out.stderr !== void 0 && out.stderr !== '') {
        return Promise.reject(out.stderr);
  ...
  git.getRunner = function getRunner() {
    return childProcess.exec('"git config --get ' + git.config.runner + '"', {
      cwd: git.target
    }).then(function(out) {
      if(out.stderr !== void 0 && out.stderr !== '') {
        return Promise.reject(out.stderr);
  ...
  git.gitContinue = function gitContinue() {
    return new Promise(function(resolve, reject) {
      ...
      console.log('git runner', git.runner);
      if (git.runner === 'none') {
